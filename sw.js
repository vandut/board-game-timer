...
// sw.js
const CACHE_NAME = 'board-game-timer-cache-v3'; // Incremented version
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then((cache) => {
        ...
        cache.add(tailwindRequest).catch(err => console.warn('Failed to cache Tailwind (no-cors):', err));
      ...
      .then((cachedResponse) => {
        ...
        return fetch(request).then(
          (networkResponse) => {
            ...
            if (networkResponse && networkResponse.ok && (networkResponse.type === 'basic' || networkResponse.type === 'opaque')) {
