...
const PlayerCountScreen: React.FC<PlayerCountScreenProps> = ({
  onProceedToTimeConfig,
}) => {
  const [numPlayersInternal, setNumPlayersInternal] = useState<number>(4); // Validated internal number
  const [numPlayersDisplayValue, setNumPlayersDisplayValue] =
    useState<string>('4'); // String value for input
  ...
  const handleNumPlayersDisplayChange = (
