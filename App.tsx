...
type TimeConfigInitialSettings = Partial<
  Omit<GameSettings, 'numberOfPlayers'>
> & {
...
const App: React.FC = () => {
  const [currentScreen, setCurrentScreen] = useState<
    'playerCount' | 'timeConfig' | 'playerNames' | 'game'
  >('playerCount');
  ...
  const [pendingTimeConfigSettings, setPendingTimeConfigSettings] =
    useState<TimeConfigInitialSettings | null>(null);
  ...
  const initializePlayers = useCallback((settings: GameSettings) => {
    const totalSessionSeconds =
      settings.totalSessionTimeHours * 3600 +
      settings.totalSessionTimeMinutes * 60;

    let playerTotalTimeAllocated = 0;
    if (settings.numberOfPlayers > 0) {
      playerTotalTimeAllocated = Math.floor(
        totalSessionSeconds / settings.numberOfPlayers
      );
    } else {
    ...
    if (settings.numberOfPlayers > 0 && settings.numberOfRounds > 0) {
      playerRoundTimeAllocated = Math.floor(
        totalSessionSeconds / settings.numberOfPlayers / settings.numberOfRounds
      );
    } else if (settings.numberOfPlayers > 0) {
      playerRoundTimeAllocated = playerTotalTimeAllocated;
    } else {
      playerRoundTimeAllocated =
        totalSessionSeconds /
        (settings.numberOfRounds > 0 ? settings.numberOfRounds : 1);
    }
  ...
  const handleProceedToTimeConfig = useCallback(
    (numPlayers: number) => {
      if (gameSettings && gameSettings.numberOfPlayers === numPlayers) {
        ...
        setPendingTimeConfigSettings({
          ...
          carryOverUnusedTime:
            gameSettings.carryOverUnusedTime !== undefined
              ? gameSettings.carryOverUnusedTime
              : true,
          payOverdueWithUnusedRoundTime:
            gameSettings.payOverdueWithUnusedRoundTime !== undefined
              ? gameSettings.payOverdueWithUnusedRoundTime
              : true,
        });
      } else {
  ...
  const handleSaveTimeSettings = useCallback(
    (timeSettings: Omit<GameSettings, 'numberOfPlayers'>) => {
      if (
        !pendingTimeConfigSettings ||
        pendingTimeConfigSettings.numberOfPlayers === undefined
      ) {
        console.error(
          'Error: Number of players is missing in pending settings.'
        );
        handleResetGame();
        return;
      }
      ...
      const completeGameSettings: GameSettings = {
        ...
        carryOverUnusedTime:
          numRounds > 1
            ? timeSettings.carryOverUnusedTime !== undefined
              ? timeSettings.carryOverUnusedTime
              : true
            : false,
        payOverdueWithUnusedRoundTime:
          numRounds > 1
            ? timeSettings.payOverdueWithUnusedRoundTime !== undefined
              ? timeSettings.payOverdueWithUnusedRoundTime
              : true
            : false,
      };
  ...
  const handleConfirmPlayerNamesAndStartGame = useCallback(
    (customizedPlayers: Player[]) => {
      setPlayers(customizedPlayers);
      if (gameSettings) {
        const totalSessionSeconds =
          gameSettings.totalSessionTimeHours * 3600 +
          gameSettings.totalSessionTimeMinutes * 60;
  ...
  const handlePlayerSelect = useCallback((playerId: number) => {
    setActivePlayerId((prevActiveId) =>
      prevActiveId === playerId ? null : playerId
    );
  }, []);

  const handlePlayerActiveTick = useCallback(
    (playerId: number) => {
      ...
      const willBeRoundOverdue =
        playerStateBeforeTick.roundTimeRemaining - 1 < 0;
  ...
  useEffect(() => {
    const totalOverdue = players.reduce(
      (sum, player) => sum + player.accumulatedOverdueTime,
      0
    );
  ...
  const handleAdvanceRound = useCallback(() => {
    ...
    const updatedPlayersData = players.map((p) => {
      let currentUnusedRoundTime =
        p.roundTimeRemaining > 0 ? p.roundTimeRemaining : 0;
      let newAccumulatedOverdue = p.accumulatedOverdueTime;
      ...
      if (
        gameSettings.payOverdueWithUnusedRoundTime &&
        currentUnusedRoundTime > 0 &&
        newAccumulatedOverdue > 0
      ) {
        const timeToPayOverdue = Math.min(
          currentUnusedRoundTime,
          newAccumulatedOverdue
        );
    ...
    setSessionTimeRemaining((prev) =>
      Math.max(0, prev + totalSessionTimeAdjustment)
    );
  ...
  if (currentScreen === 'playerCount') {
    return (
      <PlayerCountScreen onProceedToTimeConfig={handleProceedToTimeConfig} />
    );
  }
  ...
  if (currentScreen === 'game' && gameSettings && players.length > 0) {
    return (
      <GameScreen
        ...
        onAdvanceRound={
          gameSettings.numberOfRounds > 1 ? handleAdvanceRound : undefined
        }
      />
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center text-xl text-red-500 p-4 text-center">
      Error: Invalid application state. Game settings or players might be
      missing. Please{' '}
      <button
